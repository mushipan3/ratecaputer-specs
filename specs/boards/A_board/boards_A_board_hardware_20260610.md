...
CART_READY─┤ 2. PA1   PA3 7 ├─── MOSFETゲート制御（10kΩでVBATへPU）【基板内完結】
  /BEEP    │               │
  INT_SDA ─┤ 3. PA2   PA0 6 ├─── LOCAL_UPDI_A（エッジパッド経由UIAPduino PD0へ）【変更】
...
#### 1. PA1：CART_READY兼アンサーバックBEEP（1ピン2役）
...
1. UIAPduinoがI2C経由でATtiny202に「初期化完了」を通知する
1. ATtiny202がPA1をHIGH出力する（CART_READY）
1. キーボードがCART_READY HIGHをポーリングまたはHIGHエッジで検出し、ハンドシェイクを開始する
1. ハンドシェイク完了後、ATtiny202がPA1をTCA0（Type A Timer）PWM出力に切り替えてアンサーバックBEEPを吹鳴する（おまけ）
...
#### 5. PA0：LOCAL_UPDI_A（エッジパッド経由・UIAPduino PD0へ基板間接続）【変更】

A基板のATtiny202のUPDIピン。エッジパッド（Pad 8）経由でUIAPduino PD0と基板間接続する。他デバイスへは露出させない。UIAPduinoからATtiny202ファームウェアの自律書き換え（セルフアップデート）を可能にする。

**兼用制約：** UIAPduino PD0はe-ink版ではBUSY_EINK、TFT版ではGPIO4_CSとしても使用する。ATtiny202アップデート中はe-ink描画コマンドを送らないこと。TFT版では拡張デバイスのCSをネゲートすること。

なお、UIAPduino PD6はATtiny1604（D基板）のLOCAL_UPDI_Dとして使用する。PD0（LOCAL_UPDI_A）とPD6（LOCAL_UPDI_D）は独立した別線であり、UPDIは1対1接続のため同時アップデートはできない。
...
### PA0: LOCAL_UPDI_A (Edge pad -> UIAPduino PD0) [CHANGED]
- U1 PA0 (Pin6) -> Edge Pad 8 (LOCAL_UPDI_A)
- Edge Pad 8 connects to UIAPduino PD0 via board-to-board wiring.
- NOT exposed to any other external connection.
- Constraint: During ATtiny202 update, UIAPduino must NOT send e-ink draw commands
  (e-ink version) or must negate SPI expansion device CS (TFT version).
- NOTE: UIAPduino PD6 is separately used for LOCAL_UPDI_D (ATtiny1604 / D-board).
  UPDI is point-to-point. Simultaneous update of both devices is not possible.

## 4. Edge Pad Assignment [CHANGED]
- Pad 1 : VBAT_IN
- Pad 2 : GND_IN
- Pad 3 : VCC_3V3_OUT
- Pad 4 : VCC_5V_OUT
...
- Pad 8 : LOCAL_UPDI_A  (= ATtiny202 PA0, connects to UIAPduino PD0)  [CHANGED]
```
...
### LOCAL_UPDI_Aの接続方式【変更】

PA0（LOCAL_UPDI_A）はエッジパッド（Pad 8）として基板端に露出し、UIAPduino PD0へ基板間配線で接続する。Rev.2.1では「UIAPduino UPDIピン」と記載していたが、UIAPduino PD0（BUSY_EINK/GPIO4_CS兼用ピン）に割り当てを確定した。

UPDIはシングルワイヤ・1対1接続専用プロトコルであり、複数デバイスのバス接続はできない。ATtiny1604（D基板）はUIAPduino PD6（LOCAL_UPDI_D）で別途独立して接続する。同時アップデートは不可能であり、A基板とD基板は順番にアップデートする。

（その他の設計根拠はRev.2.1から変更なし）
...
|2026-06-10|Rev.2.1|基板サイズを10mm×28mm→9.5mm×27.8mmに修正                                                      |
|2026-06-10|Rev.2.1|PA1の信号名をLtcBus_SYS_INT_SPK→CART_READYに修正。CAR-01規格拡張コネクタ上列1と同一信号であることを明記              |
|2026-06-10|Rev.2.1|PA1 CART_READY動作シーケンス修正。GPIO LOW出力による挿入通知を削除。UIAPduinoからI2C通知を受けてHIGH出力する正しいシーケンスに変更|
|2026-06-10|Rev.2.1|PA0 LOCAL_UPDIをエッジパッド経由UIAPduino基板間接続に変更。エッジパッドを7→8個に変更                             |
|2026-06-10|Rev.2.2|PA0をLOCAL_UPDIからLOCAL_UPDI_Aに改名（ATtiny1604用LOCAL_UPDI_Dとの区別）【変更】                    |
|2026-06-10|Rev.2.2|LOCAL_UPDI_Aの接続先をUIAPduino PD0に確定（BUSY_EINK/GPIO4_CS兼用・アップデート中は描画停止/CS解放が必要）【変更】     |
|2026-06-10|Rev.2.2|Pad 8の説明をLOCAL_UPDI_A（UIAPduino PD0接続）に更新【変更】                                       |
|2026-06-10|Rev.2.2|UPDIは1対1接続専用・ATtiny1604はPD6（LOCAL_UPDI_D）で別途接続・同時アップデート不可を明記【変更】                    |
|2026-06-10|Rev.2.2|Pad5/Pad6のコメントに共通I2Cバス（UIAPduino・ATtiny1604・CH1115 OLED共有）を明記【変更】              |
