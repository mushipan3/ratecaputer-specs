...
4. [回路設計](#4-回路設計)
   ...
   - 4.6 ヘッドフォンジャック（J1: PJ-393-7A）
   - 4.7 SPIバス接続
5. [部品一覧（BOM）](#5-部品一覧bom)
6. [回路図自動生成指示書（Claude Code用）](#6-回路図自動生成指示書claude-code用)
7. [外部接続・インターフェース](#7-外部接続インターフェース)
   - 7.1 外部接続インターフェース
   - 7.2 ケース外部インターフェース
8. [ファームウェア仕様（YMF825制御）](#8-ファームウェア仕様ymf825制御)
   - 8.1 SPI通信パラメータ
   - 8.2 初期化シーケンス
   - 8.3 ボリュームコントロール
   - 8.4 CPU使用率（参考）
...
SPOUT1/2をLINEOUTとして使用し、MCP6001で差動→SE変換後にPAM8302Aへ送る構成とする。AUDIO_IN（アナログ側エッジパッド）からの外部音声入力はR_PWM経由でAUDIO_MONO_MIXへ混合する。AUDIO_INパッドは1本のみであり、PWM入力とアナログ直入力の**同時接続はできない**。UIAPduino PC3（PWM出力）を接続する場合はJP1をオープンにしてRC-LPF経由とし、mp3デコーダ等のアナログ音声を直接接続する場合はJP1をショートしてRC-LPFをバイパスする。JP1のショートははんだブリッジ・0Ω抵抗実装・外付けスイッチのいずれかで実現できる。外付けスイッチを使用する場合はJP1パッドに両端を接続し、基板上のJP1は未実装のままとする。どちらか一方を選択して接続すること。
...
| ショート（固定） | JP1にはんだブリッジまたは0Ω抵抗を実装 | mp3デコーダ等のアナログ音声を直接接続（固定） |
| ショート（可変） | JP1未実装のままJP1パッドに外付けスイッチを接続 | PWM入力とアナログ直入力を手動で切り替え |

外付けスイッチはJP1の2パッドに両端を接続する。FRISKケース側面等に小型スライドスイッチ（SS-12D00系相当）を設けるのが現実的。

> ⚠️ **AUDIO_INはどちらか一方のみ接続すること。** PWM入力とアナログ直入力の同時接続は不可。アナログ出力の低インピーダンスがPWMパルスを潰し、意図しない動作の原因となる。
...
| 項目 | 仕様 |
|---|---|
| 型番 | PJ-393-7A（LCSC: C128990） |
| 形式 | 3極（TRS）・NCスイッチ付き・7ピン |
| 配置 | B基板長辺（27.8mm側）端・プラグ挿入口をケース外へ突出 |
| フットプリント | 14.5×6mm（SMD） |
| 実装 | JLCPCB PCBA委託（SMD） |
...
| J1 | 3.5mmステレオミニジャック NCスイッチ付き（PJ-393-7A） | SMD 7ピン | C128990 | Basic | 1 |
...
| X1 | 水晶振動子 ABM8G-12.288MHZ-B4Y-T | SMD 3225-4P | CL=10pF・DigiKey調達・手はんだ（JLCPCBに12.288MHz・3225品の安定在庫なし・PCBA不可） |
...
- J1 : PJ-393-7A, 3.5mm Stereo Mini Jack, 7-Pin SMD
        Switch type: Pattern-A NC
        (Tip shorted to Switch pin when plug absent;
         opens upon plug insertion)
        LCSC: C128990                       -> JLCPCB PCBA Target
...
YMF825のマスターボリュームはレジスタ操作（アドレス0x19・8bit）でソフトウェア制御する。

**制御経路：**
```
ATtiny1604（ジョイスティック上下検知）
    │  I2C
    ↓
UIAPduino（CH32V003）
    │  SPI（CS_YMF=PD2）
    ↓
YMF825 マスターボリュームレジスタ（0x19）
```

**実装方針：**
- ATtiny1604がジョイスティック上下操作を検知し、I2C経由でUIAPduinoに音量変更コマンドを送信する
- UIAPduinoが受信コマンドに従いYMF825レジスタを更新する
- 音量変更コマンドの定義はD基板ファームウェア仕様およびシステム機能目論見書（features/system_menu）に準拠する
- 物理ボリュームつまみは使用しない（FRISKケース面積制約・ATtiny1604からのデジタル制御で代替）

**スピーカーミュート：**
J1（PJ-393-7A）のNCスイッチ機構によりハードウェアで自動制御する。イヤホン未挿入時はNCスイッチが閉じてPAM8302A IN-がGNDに接地されスピーカーが鳴る。イヤホン挿入時はNCスイッチが開いてIN-がフローティングになりスピーカー出力が自動消音される。GPIO制御・物理スイッチ・ソフトウェア処理は不要。詳細は§4.5参照。
...
当初MJ-495A（秋月電子・スルーホール・14.3×12mm）を採用していたが、以下の検討を経てPJ-393-7A（SMD・14.5×6mm）に変更した。

**検討経緯：**
1. スイッチ機能なし小型ジャック（SJ-3523-SMT系・6×6mm）への変更を検討したが、挿入検出手段がなくなりユーザーの手動切り替えUIが必要になる問題が発覚
2. 一時的に「スピーカーケーブルに物理スイッチを設ける」方式を検討したが、JLCPCBでSMD品を探索したところPJ-393-7A（LCSC: C128990）がNCスイッチ付き7ピン品であることが判明。物理スイッチは不要となった
3. PJ-393-7AはNCスイッチ付きのため現行のハードウェア完結ミュート回路（C5経由GND接地）をそのまま活用できる。さらにSMDのためPCBA委託が可能となり手はんだ工数も削減できる

**結論：** PJ-393-7A採用によりMJ-495A比で奥行きが12mm→6mmに半減し、かつ手はんだ対象から外れPCBA委託となる。ハードウェアミュート回路も現行維持。

### ボリュームコントロール方式選定根拠

物理ボリュームつまみ（Alps RK10J12E0A0A等）はB基板（19mm×27.8mm）の面積制約上搭載不可能（本体フットプリントだけで約10×12mmを占有）。デジタルポテンショメータ（I2C/SPI制御IC）は追加部品・配線が必要。YMF825のマスターボリュームレジスタ（0x19）をATtiny1604→I2C→UIAPduino→SPI経由で操作する方式が追加部品ゼロで最もシンプルかつFRISKケース制約に適合すると判断した。

### PAM8302A外付けアンプ採用による音質的評価

YMF825の内蔵BTLアンプを使用せずPAM8302Aを外付けする構成は、音質面でポジティブな方向に働く。

| 項目 | YMF825内蔵BTLアンプ | PAM8302A外付け（本設計） |
|---|---|---|
| デジタルノイズ混入 | 同一チップのためDACコアと電源共用・ノイズが乗りやすい | 物理的に分離・AMP_5.0V独立電源で動作 |
| ノイズ除去 | なし | MCP6001差動→SE変換でCMRR約40dBのノイズキャンセル後に入力 |
| 設計自由度 | 固定 | スピーカー特性に合わせた最適化が可能 |

YMF825Boardは内蔵BTLアンプをそのまま使う設計だが、本設計はMCP6001+PAM8302Aという独立した信号経路を構築しており、「越えるべき目標：YMF825Board音質同等以上」に対して理論上ポジティブな方向に働く。ただしYMF825自体の残留ノイズ（−85dBV）がボトルネックであるため、劇的な差ではなく「同等以上」の範囲での改善となる。

### AUDIO_IN排他制約（同時接続不可）

AUDIO_INパッドは1本のみであり、PWM入力とアナログ直入力の同時接続はできない。アナログ出力の低インピーダンスがPWMパルスの波形を潰し、意図しない動作の原因となるため。将来的にPWMとアナログを同時混合したい場合は独立したミックス抵抗を追加する回路変更が必要。

### 水晶振動子X1のJLCPCB PCBA不可について

JLCPCBには12.288MHz・SMD 3225-4P品の安定在庫がなく（C9900037065は購入不可・アセンブリ参照のみ）、PCBA委託は実用上不可能。ABM8G-12.288MHZ-B4Y-TをDigiKeyで調達して手はんだする方針が最も確実。12.288MHzはオーディオ用の特殊周波数（44.1kHz系クロック源）であり、一般的な電子部品店では取り扱いが少ない。
...
| 2026-06-10 | Rev.2.9 | J1をMJ-495A（手はんだ・スルーホール）からPJ-393-7A（PCBA・SMD・LCSC: C128990）に変更。手はんだ4点→3点。PCBA 23点→24点。§4.6・§5・§6・§7.2・設計根拠メモを更新。§8.3にボリュームコントロール（ATtiny1604→I2C→UIAPduino→SPI→YMF825レジスタ0x19）を追記。§8.4にCPU使用率を移動。設計根拠メモにジャック選定経緯とボリューム方式選定根拠を追記。 |
| 2026-06-12 | Rev.2.10 | §8.3スピーカーミュートをPJ-393-7A NCスイッチによる自動消音（ハードウェア完結）に修正。物理スイッチ方式の誤記を削除。設計根拠メモのジャック選定経緯から物理スイッチ検討ステップを整理。 |
| 2026-06-12 | Rev.2.11 | §2.2・§4.2にAUDIO_IN排他制約（PWM/アナログ同時接続不可）を明記。§5 BOM X1にJLCPCB PCBA不可の理由を追記。§10設計根拠メモにPAM8302A音質評価・AUDIO_IN排他制約・水晶X1 PCBA不可根拠を追加。 |
| 2026-06-12 | Rev.2.12 | §2.2・§4.2 JP1に外付けスイッチによる手動切り替え方式を追加。JP1切り替え表を3行に拡張（オープン・固定ショート・外付けスイッチ）。外付けスイッチ接続方法とケース実装例を記載。 |
