...
9.5mm × 27.8mmのスティック形状は、A基板（電源管理：9.5×27.8mm）・B基板（FM音源：19×27.8mm）と縦幅を統一しており、3基板をV-CUTのみで切り離せる格子状パネルを構成する。
...
| 項目 | 仕様 |
|---|---|
| 基板サイズ | 9.5mm × 27.8mm（縦27.8mmはA基板・B基板とパネライズグリッドを統一） |
...
| 2026-06-12 | Rev.1.3 | パターンZ章立てに全面再編。§1「本書の位置づけ」新設（参照ドキュメント表追加）。§2「概要・目的」に独立。§3「物理仕様」に独立。§4「回路設計」にストレージアーキテクチャとコアアーキテクチャを統合。§6「外部接続・インターフェース」をClaude Code指示書から分離・CAR-01規格との整合確認表を統合。§9「未決事項（PENDING）」新設（PENDINGなしと明記）。目次追加。Claude Code投入用表記を削除。 |
