...
> ## ★Rev.2.4 の位置づけ — MCU を ATtiny404 へ変更（型番のみ・回路は未再設計）
> **本 Rev で確定したのは MCU 型番の変更だけである。** 回路・ピン割当・BOM・レイアウトは
> Rev.2.3 のまま（ATtiny404・SOIC-8 前提）で残っている。**そのままでは製造できない。**
> 該当箇所には `★TODO(Rev.2.5)` を付けて可視化した。次版で回路設計を行うこと。
>
> ### なぜ ATtiny404 なのか（2026-08-09・採用根拠）
> 下記2つをやろうとすると **A基板の出力ピンが足りない**、という結論に至ったため:
> 1. **主基板の電源をスリープまたは断にしても、D基板だけでサブディスプレイと
>    ジョイスティックで遊べる柔軟性**を確保する。→ 下流レールを選択的に OFF する
>    EN 制御ピンが要る（下記 PENDING 2 に対応）。
> 2. **B基板の FM音源用 3.3V を、主基板からの供給ではなく品質確保のため
>    A基板から直接供給する。** → 追加の電源制御ピンが要る。
>
> ∴ ATtiny404（8ピン・全使用済み）では不可能で、**ATtiny404（14ピン）を採用**した。
> ★これは下記 PENDING 注記 2・3 が指摘していた宿題への回答である。
>
> ## ★PENDING注記（2026-07-27・電源協調検討で判明した齟齬・回路は未変更のまま）
> 主基板の電源協調（#6電源断/#7スリープ/SLEEP2）を検討した結果、Rev.2.3設計と以下の齟齬が判明した。
> **Rev.2.4 で MCU を替えたが回路は未再設計。3 はピン増で解ける見込み、2 はレール分離の設計が要る**:
> 1. **実バッテリ＝300mAh LiPo**（本書は §8「対象400〜700mAh」前提）。→ **充電300mA＝1.0C** となり密閉フリスク筐体で狙いの0.5Cより高く発熱リスク。**R_PROG見直し**（例 150mA・R_PROG≈6.6kΩ）を要検討。
> 2. **SLEEP/SLEEP2（省電力・D基板単体プレイ）が現トポロジで不可**: MOSFET(U2)1個の後段が全部ハードワイヤON（Buck U3・MT3608 U4 の EN は SWITCHED_VBAT直結／アンプ PAM8302A の SD は常時プルアップ）で**下流を選択的にOFFできない**。かつ D基板は B基板と同じ Buck 3.3Vレール。→ 独立レール制御が必要。
> 3. ~~**ATtiny404 の8ピンは全使用済み**~~ **→ Rev.2.4 で ATtiny404（14ピン）へ変更し解消見込み。** 旧記述:（PA0=UPDI/PA1=CART_READY・BEEP・CHG/PA2・PA4=I2C/PA3=MOSFET/PA5=電池分圧+ボタン）＝**EN制御を足す空きピンが無い**。SLEEP実装にはMCUピン増（例 ATtiny412/1604）が要る。
> 4. **機械マスタースイッチが無い**（VBAT_IN→ATtiny404 VCC直結）＝真の全電源断(TRUE-OFF)不能・最下位はSTANDBY。加えて**バッテリ分圧 R2+R3(100k+100k)が常時≈18.5µA**（ゲートなし・300mAhで約1.8年でカラ）。低STANDBY化（分圧ゲート化/高抵抗/ADC間欠）と機械SW追加は Rev.2.4 で検討。
>
> 詳細・設計判断の経緯は開発側の電源協調memo（`dboard_role_and_protocol_prospectus` §3.2.3論点）を参照。

## 1. 実装目論見書

### プロジェクト概要

本ボード（A基板）は、ラテカピュータ（CAR-01規格）に準拠した自律型電源管理モジュール基板である。ATtiny404をコプロセッサとして搭載し、メインMCU（UIAPduino等）のSRAM・FRAM・CPU時間を一切消費せずに、バッテリー残量監視・充電状態監視・電源シーケンス制御・接続通知BEEPを実現する。9.5mm × 34mmの片面実装スティック形状に全回路を収める。

### 物理仕様・製造要件

| 項目 | 仕様 |
|---|---|
| 基板サイズ | 9.5mm × 34mm（縦34mmはC基板・D基板とパネライズグリッドを統一。B基板のみフリスクケース格納制約により27.8mmを維持） |
...
- **JLCPCB PCBA 自動実装：** ATtiny404・IRLML6402・Buck DCDC IC・MT3608・MCP73831・受動部品（1608サイズ）の全品
- **手はんだ：** なし（全点工場実装）
...
**常時通電ライン（VBAT_IN）：** リポからATtiny404の1番ピン（VCC）へ直結。電源OFF時はATtiny404がパワーダウンスリープ（数µA）で待機し、バッテリー消費を最小化する。ATtiny404の動作電圧範囲は1.8〜5.5Vであり、リポ放電末期（3.0V）でも問題なく動作する。

**スイッチド下流ライン：** ATtiny404の7番ピン（PA3）がLOWを出力するとP-ch MOSFET（IRLML6402・U2）が導通し、下流のDCDC回路へ電流が流れる。
...
VBAT_IN → IRLML6402（U2）
    ...
    │
    └── MT3608（U4・5.3V設定）
            ├──[D_AMP: B5819W]── VCC_5V_OUT → B基板 PAM8302A・YMF825 SPVDD（5.0V到達）
            └── UIAPduino 5Vピン（入力）
                    └── UIAPduino内部レギュレータ → 3.3V出力
                                → C基板（W25Q128JV・MB85RS4MT）
```

**給電独立性の方針：** A・B・D基板はUIAPduinoに依存しない独立給電。C基板のみUIAPduino 3.3V出力経由。これにより将来のMCU変更（Xiaoシリーズ等）時も電源設計の変更が不要となる。

**充電ライン（Diode-OR合流）：** 2系統の5V入力をDiode-ORでMCP73831 VDDに合流する。

```
USB_5V_IN（Pad 9）──[D_CHG_USB: B5819W]──┐
                                           ├── MCP73831 VDD → VBAT_IN（充電）
5V_IO（拡張コネクタ下列1）──[D_CHG_EXT: B5819W]──┘
```

- USB給電時：D_CHG_USBが導通、D_CHG_EXTは逆バイアス
- 上位デバイスから5V_IO受電時：D_CHG_EXTが導通、300mAでLiPoを充電
- 同時の場合：電圧が高い方が自動的に導通（フールプルーフ・ソフトウェア制御不要）

**PWR_IO自動切替ライン（拡張コネクタ下列3）：** 5V_IOの電位をハードウェア検出し3.3V/5Vを自動切替する。

```
Buck 3.3V出力──[Q_PWR: IRLML6402]── PWR_IO（下列3）
                 │Gate                    │
                 ├[R_GPWR: 100kΩ]─GND    │
                 └──── 5V_IO（下列1）    │
                                          │
...
- 5V_IO = 0V（バッテリー動作中）：Q_PWR ON → PWR_IOに**3.3V**出力
- 5V_IO = 5V（上位から受電中）：Q_PWR OFF、D_PWR5導通 → PWR_IOに**約4.7V（5V相当）**出力
...
```
              ATtiny404 (SOIC-8) ★TODO(Rev.2.5): ATtiny404 は SOIC-14。パッケージ・フットプリント要変更
           ┌───────┴───────┐
...
2. UIAPduinoがI2C経由でATtiny404に「初期化完了」を通知する
3. ATtiny404がPA1をHIGH出力する（CART_READY）
4. キーボードがCART_READY HIGHをポーリングまたはHIGHエッジで検出し、ハンドシェイクを開始する
5. ハンドシェイク完了後、ATtiny404がPA1をTCA0（Type A Timer）PWM出力に切り替えてアンサーバックBEEPを吹鳴する（おまけ）
...
ATtiny404はPA5 ADCによるバッテリー電圧（残量%）とPA1のSTAT読み取り（充電中/完了）を組み合わせ、UIAPduinoからのReadリクエストに対して充電状態を含む情報を返答する。
...
INT_SDA / INT_SCLとして、対外14ピン拡張コネクタ（UIAPduino・ATtiny1604・CH1115 OLEDと共通バス）に接続する。ATtiny404は**I2Cスレーブ**として動作し、UIAPduinoからのReadリクエストに対してバッテリー残量値・充電状態を返答する。

#### 5. PA0：LOCAL_UPDI_A（エッジパッド経由・UIAPduino PD6へ基板間接続）

A基板のATtiny404のUPDIピン。エッジパッド（Pad 8）経由でUIAPduino PD6と基板間接続する。他デバイスへは露出させない。UIAPduinoからATtiny404ファームウェアの自律書き換え（セルフアップデート）を可能にする。
...
MT3608（SOT-23-6、Vin 2V〜24V、Iout最大2A）によるYMF-825音源モジュール駆動用昇圧回路。出力電圧は**5.3V設定**（D_AMPのVF降下補償により、B基板に5.0Vが届く）。出力電流目安300mA。試作時は9.5mm×27.8mm基板の5V出力パッドから空中配線で外付けモジュールへ接続する割り切りスタイルを採用し、量産時はSMDチップ単体設計を優先する。
...
**MCP73831を選定した理由：**
LiPo充電ICの主要候補としてTP4056・CN3058E・MCP73831を比較検討した。TP4056はJLCPCB Basicで段取り費$0の優位があるが、状態出力ピンが2本（CHRG・STDBY）であり、ATtiny404の空きピンが1本（PA1）しかないため採用不可。CN3058EもSOT-23-6で小型だが同様に状態ピン2本で不適。MCP73831はSTATピン1本で充電中・充電完了の両状態を区別でき（充電中LOW・完了ハイインピーダンス）、SOT-23-5で最小サイズ、最大500mAで今回の300mA設定に適合する唯一の候補であった。Extended Parts（+$3）となるが設計制約上代替なし。
...
| STAT接続先 | ATtiny404 PA1（GPIO入力・内部プルアップ・通常動作時） |
| VDD接続先 | D_CHG_USBカソード ＋ D_CHG_EXTカソード（Diode-OR合流点） |
| VBAT接続先 | VBAT_IN（LiPoバッテリーライン） |
| USB未接続・5V_IO無入力時 | VDD = 0V → 充電IC自動停止・バッテリーからの逆流なし（内部保護） |

#### 9. Diode-OR回路（電源衝突防止・充電経路合流）

複数の5V入力源をDiode-ORで安全に合流させる。ダイオードは全てB5819W（SOD-123・JLCPCB Basic）で統一。

| RefDes | 素子 | アノード | カソード | 役割 |
|---|---|---|---|---|
| D_CHG_USB | B5819W | USB_5V_IN（Pad 9） | MCP73831 VDD | USB充電経路・逆流防止 |
| D_CHG_EXT | B5819W | 5V_IO（拡張コネクタ下列1） | MCP73831 VDD | 上位デバイス受電充電経路 |
| D_AMP | B5819W | MT3608出力（VCC_5V_OUT） | B基板AMP_5.0Vライン | B基板給電・逆流防止 |
| D_PWR5 | B5819W | 5V_IO（拡張コネクタ下列1） | PWR_IO（拡張コネクタ下列3） | PWR_IO 5Vモード経路 |

**Diode-OR動作原理：** 複数電源をダイオード経由で合流させると最高電位の電源のみが導通し、他は逆バイアスで自動遮断される。これにより起動順序・操作手順に依存しないフールプルーフ電源設計を実現する。ソフトウェア制御不要・完全ハードウェア保護。

**MT3608出力電圧5.3V設定の根拠：** D_AMPのVF（B5819W、約0.3V）降下を補償し、B基板に5.0Vを届けるため。

#### 10. PWR_IO電圧自動切替回路

5V_IO（拡張コネクタ下列1）の電位をハードウェア検出し、拡張デバイスへの供給電圧を3.3V/5Vで自動切替する。

| RefDes | 素子 | 役割 |
|---|---|---|
| Q_PWR | IRLML6402（SOT-23・U2と同型） | 3.3Vラインのロードスイッチ |
| R_GPWR | 100kΩ（1608・U2と同型） | Q_PWRゲートGNDプルダウン |
| D_PWR5 | B5819W（上記§9に記載） | 5V_IO → PWR_IO 5Vモード経路 |

**動作：**
- 5V_IO = 0V：R_GPWRがGateを0Vに引く → VGS = −3.3V → Q_PWR ON → **PWR_IOに3.3V出力**
- 5V_IO = 5V：Gate = 5V → VGS = +1.7V → Q_PWR OFF、D_PWR5導通 → **PWR_IOに約4.7V出力**

**PWR_IO仕様：**

| 項目 | 仕様 |
|---|---|
| 出力電圧（3.3Vモード） | 3.3V（Buck DCDC出力直結） |
| 出力電圧（5Vモード） | 約4.7V（5V_IO − D_PWR5 VF） |
| 最大供給電流 | **100mA**（拡張デバイス規格上限・ユーザー開示スペック） |
| 自動切替トリガー | 5V_IO電位（ハードウェア自動・ソフトウェア不要） |
| 対象接続先 | ラテカナノ・拡張デバイス・M5Stack系3.3V/5V Unit・Groveデバイス等 |

**PWR_IO 100mA上限の根拠：** 想定接続デバイス（センサー系・小型マイコン・CardKB系カスタムキーボード）の最大消費電流は55mA程度。100mAは十分な余裕を持つ上限値として設定し、ESP32系（最大240mA）やモーター系など大電流デバイスは対象外とする。

#### 9. ATtiny404ファームウェアの役割（確定版）

ATtiny404のファームウェアは以下の機能を担う。OLEDへの描画処理は一切含まない。
...
- U1  : ATtiny404 (SOIC-8) ★TODO(Rev.2.5): ATtiny404 は SOIC-14。パッケージ・フットプリント要変更               LCSC: C2052970 ★TODO(Rev.2.5): これは ATtiny202 の型番。ATtiny404 の LCSC 番号へ要更新  Extended +$3
...
- C7  : 0.1uF / 10V / X7R (1608)        ATtiny404 VCC bypass
...
- U5 STAT (Pin3) -> U1 PA1 (Pin2)
  (Open-drain output. Internal pull-up on ATtiny404 PA1 active during normal operation.
...
- ATtiny404 operates as I2C SLAVE.
...
- Pad 8 : LOCAL_UPDI_A  (= ATtiny404 PA0, connects to UIAPduino PD6)
...
- U1 (ATtiny404 SOIC-8): place near board center.
...
### 🟢 自動実装（JLCPCB PCBA委託）対象パーツ　計29点

| RefDes | 部品名・仕様 | パッケージ | LCSC番号 | 区分 | 数量 | 役割 |
|---|---|---|---|---|---|---|
| U1 | ATtiny404マイコン | ★TODO(Rev.2.5) SOIC-14 | ★TODO 要更新 | Extended | 1 | 電源管理コプロセッサ |
| U2 | IRLML6402 P-ch MOSFET | SOT-23 | C5148470 | Extended | 1 | スイッチド電源ライン制御（PA3制御） |
| U3 | Buck DCDC 3.3V固定（RT8008-33GB同等） | SOT-23-5 | 要確認 | Extended | 1 | 3.3V降圧・リポ放電末期も安定 |
| U4 | MT3608 昇圧DCDC | SOT-23-6 | C84817 | Extended | 1 | 5.3V昇圧・YMF-825/UIAPduino駆動用 |
| U5 | MCP73831T-2ACI/OT LiPo充電IC | SOT-23-5 | 要確認 | Extended | 1 | LiPoバッテリー充電・300mA |
| Q_PWR | IRLML6402 P-ch MOSFET | SOT-23 | C5148470 | Extended | 1 | PWR_IO 3.3V/5V自動切替ロードスイッチ（U2と同型・段取り費追加なし） |
...
| R_upper | MT3608 FB抵抗・上（**780kΩ**/±1%） | 1608 | 要確認 | Basic | 1 | 5.3V出力電圧設定（VF補償） |
| R_lower | MT3608 FB抵抗・下（100kΩ/±1%） | 1608 | C25803 | Basic | 1 | 5.3V出力電圧設定 |
| R_PROG | MCP73831充電電流設定抵抗（3.3kΩ/±1%） | 1608 | C22978 | Basic | 1 | 充電電流300mA設定 |
| R_GPWR | Q_PWRゲートプルダウン抵抗（100kΩ/±1%） | 1608 | C25803 | Basic | 1 | PWR_IO 3.3Vモード時ゲートGND固定 |
...
| C7 | ATtiny404 VCCバイパスコンデンサ（0.1µF/10V/X7R） | 1608 | C14663 | Basic | 1 | ATtiny404電源デカップリング |
...
| D_CHG_USB | ショットキーダイオード（B5819W） | SOD-123 | C8598 | Basic | 1 | USB_5V_IN → MCP73831 VDD（Diode-OR） |
| D_CHG_EXT | ショットキーダイオード（B5819W） | SOD-123 | C8598 | Basic | 1 | 5V_IO → MCP73831 VDD（Diode-OR） |
| D_AMP | ショットキーダイオード（B5819W） | SOD-123 | C8598 | Basic | 1 | MT3608出力 → AMP_5.0V（逆流防止） |
| D_PWR5 | ショットキーダイオード（B5819W） | SOD-123 | C8598 | Basic | 1 | 5V_IO → PWR_IO（5Vモード経路） |
...
本基板はU1・U2・U3・U4・U5の5品種がExtended Parts扱いとなる。段取り費は5種 × $3 = **合計約$15**。5枚製造の場合、1枚あたり$3.00の上乗せとなる。Q_PWRはU2（IRLML6402）と同一品種のため段取り費追加なし。
...
UPDI書き換え中（ATtiny404のメモリ消去・再書き込み中）はPA3がハイインピーダンスになる。しかしR1（10kΩ）がゲートをVBAT_INへ物理的に引き寄せ続けるため、P-ch MOSFETはゲート＝ソース＝VBATとなりOFF状態を維持する。書き換え中に下流の電源ラインが乱れることはなく、ATtiny404のファームウェアのみを安全にセルフアップデートできる。
...
CART_READY通知とBEEPはいずれも拡張コネクタ接続時の初期化処理における一時的な役割であり、接続が成立した以降・および拡張コネクタに何も接続されていない時間帯にPA1は空く。この空き時間を充電状態監視（MCP73831 STATポーリング）に活用することで、ATtiny202の8ピンを全て使い切りながら充電状態監視を実現した（★TODO(Rev.2.5): ATtiny404 では前提が変わる）。ATtiny404はピンのモードをソフトウェアで動的に切り替えられるため、フェーズ移行時に出力→入力の切り替えを行う。

### MCP73831充電IC選定根拠

★TODO(Rev.2.5): 以下は ATtiny202（8ピン）前提の選定根拠であり、ATtiny404（14ピン）では前提が崩れている。充電IC選定を再検討すること。
...
### MT3608 フィードバック抵抗計算（Vout = 5.3V・VF補償）

MT3608のフィードバック基準電圧Vref = 0.6V。D_AMP（B5819W）のVF ≒ 0.3Vを補償するため出力を5.3Vに設定する。
```
Vout = Vref × (1 + R_upper / R_lower)
5.3 = 0.6 × (1 + R_upper / 100k)
R_upper = 100k × (5.3/0.6 - 1) = 783kΩ → 780kΩ（E96系列）
```
B基板到達電圧：5.3V − VF(0.3V) = **5.0V**（YMF825 SPVDD下限4.5Vに対して十分な余裕）

### Diode-OR設計根拠

複数の5V入力源が同時に存在した場合、電圧の高い方のダイオードのみが導通し、他は逆バイアスで自動遮断される。これにより起動順序・接続手順に依存しないフールプルーフ設計を実現する。B5819W（Basic Parts・SOD-123）を全Diode-OR素子で統一することでコストと管理を最小化した。

### PWR_IO自動切替の設計根拠

M5Stack系デバイスのGrove電源は5V、ラテカ系・多くのセンサー系は3.3Vと異なる。プロトコルネゴシエーション前に電源が確定していなければならないため、5V_IOの電位という最も低レイヤのハードウェア情報をトリガーとした自動切替を採用した。ソフトウェア依存を排除することでM5Stack系デバイスとの互換性をフールプルーフに確保する。PWR_IO 5Vモード時の約4.7Vは、Grove Unit動作電圧5V±10%（4.5〜5.5V）の範囲内であり実用上問題ない。

### PWR_IO最大供給電流100mAの根拠

想定接続デバイスの消費電流：ラテカナノ（ATtiny10+LED×3）≒20mA、CardKB（ATmega8A+RGB LED）≒30mA、カスタムキーボード（CardKB+OLED+UIAPduino+ATtiny1604）≒55mA、Grove Unitセンサー系≒50mA以下。100mAはこれら全ての想定用途に対して余裕を持つ上限値。ESP32系（最大240mA）やモーター系など大電流デバイスは5V_IO（下列1）からの直接給電を推奨し、PWR_IOの対象外とする。

### USB充電経路・5V_IO充電経路の設計根拠

USB充電（D_CHG_USB）：UIAPduinoの5VピンはUSB接続時にUSB VBUSの5Vを出力する。USB未接続時は0Vになるため、D_CHG_USBは逆バイアスとなり充電ICへの電流は流れない。バッテリーからの逆流も内部保護により防止される。

5V_IO充電（D_CHG_EXT）：上位デバイス（キーボード等）から拡張コネクタ下列1（5V_IO）経由で5Vが供給された場合、D_CHG_EXTを通じてMCP73831 VDDに入力され、自動的にLiPoへの充電が開始される。上位デバイスが切断されると0Vに戻りD_CHG_EXTが逆バイアスとなり充電停止。追加の制御回路不要。
...
| 2026-06-17 | Rev.2.3 | Diode-OR回路追加（D_CHG_USB・D_CHG_EXT・D_AMP・D_PWR5：B5819W×4）。PWR_IO自動切替回路追加（Q_PWR：IRLML6402×1、R_GPWR：100kΩ×1）。MT3608出力電圧5.0V→5.3V（D_AMP VF補償、R_upper 750kΩ→780kΩ）。各基板給電経路を明確化（B・D基板独立給電、C基板UIAPduino経由）。充電経路をDiode-ORに変更（USB_5V_IN・5V_IO双方からMCP73831 VDDへ合流）。PWR_IO（拡張コネクタ下列3）設計・最大供給電流100mAを規定。基板サイズを9.5×27.8mm→9.5×34mmに変更（実装密度改善・C基板・D基板とグリッド統一）。B基板とのパネライズ方法はPENDING。BOM 23点→29点。設計根拠メモにDiode-OR・PWR_IO・M5Stack系対応根拠を追記 |

| 2026-08-09 | Rev.2.4 | **MCU を ATtiny202 → ATtiny404 へ変更（型番のみ）。** 根拠＝(1) 主基板スリープ/断でもD基板単体で遊べる柔軟性のため下流レールの EN 制御ピンが要る、(2) B基板 FM音源用 3.3V を品質確保のためA基板から直接供給する、の2点で 8ピンでは足りないため。★回路・ピン割当・BOM・レイアウトは Rev.2.3 のまま（ATtiny202・SOIC-8 前提）で `★TODO(Rev.2.5)` を付与。充電IC(MCP73831)の選定根拠は「空きピン1本」を前提としており前提が崩れているため再検討が要る |

---

*文書バージョン：Rev.2.4（型番のみ確定・回路未再設計） / Claude Code投入前にエッジパッド数・Claude Code指示書の更新を要確認（§2は次回KiCad作業時に更新）*
