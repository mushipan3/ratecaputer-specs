...
1. [概要](#1-概要)
2. [機能一覧](#2-機能一覧)
3. [電源シーケンス制御](#3-電源シーケンス制御)
   - 3.1 電源ONシーケンス
   - 3.2 電源OFFシーケンス（長押しシャットダウン）
   - 3.3 PA5判定閾値
4. [バッテリー残量監視](#4-バッテリー残量監視)
5. [I2Cスレーブ動作](#5-i2cスレーブ動作)
6. [CART_READY通知シーケンス](#6-cart_ready通知シーケンス)
7. [アンサーバックBEEP](#7-アンサーバックbeep)
8. [変更履歴](#8-変更履歴)
...
本書はA基板搭載ATtiny404のファームウェア仕様を定める。ATtiny404はUIAPduino（CH32V003）のI2Cスレーブとして動作し、電源管理・バッテリー残量監視・CART_READY通知・アンサーバックBEEPを担う。OLEDへの描画処理は一切含まない。
...
UIAPduino・B基板が起動
    ↓
UIAPduino共通プログラム初期化完了後、I2C経由でATtiny404に通知
    ↓
ATtiny404がPA1をHIGH出力（CART_READY）
    ↓
...
VCC_3V3_OUT / VCC_5V_OUT が落ちる
    ↓
ATtiny404はパワーダウンスリープへ移行（VBAT_INで常時通電・数µA）
```
...
| コマンド種別 | 方向 | 内容 |
|---|---|---|
| Read（バッテリー残量） | ATtiny404→UIAPduino | 残量値1バイト（0〜100%） |
| Write（CART_READY通知） | UIAPduino→ATtiny404 | 初期化完了通知・PA1 HIGH出力を指示 |
...
UIAPduino共通プログラム初期化完了
    ↓
UIAPduino → I2C Write → ATtiny404（「初期化完了」通知）
    ↓
ATtiny404: PA1をHIGH出力（CART_READY = HIGH）
    ↓
キーボードがCART_READY HIGHをポーリングまたはHIGHエッジで検出
    ↓
ハンドシェイク開始・完了
    ↓
ATtiny404: PA1をTCA0（Type A Timer）PWM出力に切り替え
    ↓
アンサーバックBEEP吹鳴（おまけ）
```
...
| 2026-06-12 | Rev.1.1 目次追加。`## 変更履歴`を`## 8. 変更履歴`に番号付与。 |
