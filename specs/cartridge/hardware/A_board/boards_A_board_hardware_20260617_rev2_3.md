...
> ## ★PENDING注記（2026-07-27・電源協調検討で判明した齟齬・本Revは設計未変更）
> 主基板の電源協調（#6電源断/#7スリープ/SLEEP2）を検討した結果、本Rev.2.3設計と以下の齟齬が判明した。**現時点では設計変更せず可視化のみ**（擬似MT実装の非ブロッカーとして別トラック保留）。**将来 SLEEP/SLEEP2 を実装するなら Rev.2.4 で再設計が要る**:
...
> 3. **ATtiny202 の8ピンは全使用済み**（PA0=UPDI/PA1=CART_READY・BEEP・CHG/PA2・PA4=I2C/PA3=MOSFET/PA5=電池分圧+ボタン）＝**EN制御を足す空きピンが無い**。SLEEP実装にはMCUピン増（例 ATtiny412/1604）が要る。
> 4. **機械マスタースイッチが無い**（VBAT_IN→ATtiny202 VCC直結）＝真の全電源断(TRUE-OFF)不能・最下位はSTANDBY。加えて**バッテリ分圧 R2+R3(100k+100k)が常時≈18.5µA**（ゲートなし・300mAhで約1.8年でカラ）。低STANDBY化（分圧ゲート化/高抵抗/ADC間欠）と機械SW追加は Rev.2.4 で検討。
>
...
本ボード（A基板）は、ラテカピュータ（CAR-01規格）に準拠した自律型電源管理モジュール基板である。ATtiny202をコプロセッサとして搭載し、メインMCU（UIAPduino等）のSRAM・FRAM・CPU時間を一切消費せずに、バッテリー残量監視・充電状態監視・電源シーケンス制御・接続通知BEEPを実現する。9.5mm × 34mmの片面実装スティック形状に全回路を収める。
...
**常時通電ライン（VBAT_IN）：** リポからATtiny202の1番ピン（VCC）へ直結。電源OFF時はATtiny202がパワーダウンスリープ（数µA）で待機し、バッテリー消費を最小化する。ATtiny202の動作電圧範囲は1.8〜5.5Vであり、リポ放電末期（3.0V）でも問題なく動作する。

**スイッチド下流ライン：** ATtiny202の7番ピン（PA3）がLOWを出力するとP-ch MOSFET（IRLML6402・U2）が導通し、下流のDCDC回路へ電流が流れる。
...
*文書バージョン：Rev.2.3確定版 / Claude Code投入前にエッジパッド数・Claude Code指示書の更新を要確認（§2は次回KiCad作業時に更新）*
