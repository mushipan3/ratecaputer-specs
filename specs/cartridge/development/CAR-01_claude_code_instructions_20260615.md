...
**理由**: 恵梨沙フォント（SPI Flash 0x008000常駐）はアプリ選択UI（app_loader_run・共通プログラム内）でも**直読み**で表示する（Rev.0.6でFRAMへの転送・展開を廃止。旧記述の「FRAM 0x13836へ55KB転送」は無効）。アプリ選択画面が共通プログラム内で実行され、日本語タイトル表示に共通プログラム内の恵梨沙フォント描画関数が必要な点は不変。
...
// FRAM（共通・グレードによらず）
// ⚠2026-07-18是正: 下記の番地は旧FRAMマップ（2026-07-13以前・恵梨沙フォントFRAM展開時代）で現行と異なる。
//   現行の正本は iap.h / CAR-01_common_program_spec §5(Rev.0.6):
//     SPI Flash管理処理 0x00000 / 共通プログラムバックアップ 0x08000 / 管理領域 0x0A000 /
//     予備バッファ 0x0C000 / セーブデータ 0x1C000 / タスクスロット領域 0x20000
//     （タスクスロット内: メタ情報0x20000 / CTXスタック 0x22000 / アプリ作業領域）
//   恵梨沙フォントはFRAMに展開せずSPI Flash 0x008000から直読み（FRAM_FONT_BASE 0x13800は廃止）。
//   以下の #define は歴史参照として残す（現行コードは iap.h の定数を使うこと）。
