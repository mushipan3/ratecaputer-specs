...
SPI FRAM 2号（ログ用・開発機のみ）
  CS  → PD6（注: PD6は通常未使用。ログ用として割り当て）
  SCK → SCKライン
  MOSI→ MOSIライン
  MISO→ MISOライン
  VCC → 3.3Vレール
...
### 1.3 FRAM 2号のCS割り当て（ログ用）

FRAM 2号はPD6に接続する。これは製品版（スタンダード・プレミアム共通）の1チップFRAM構成とは異なるが、試験環境ではFRAM 2号を以下の用途に使用する。

```c
// 試験環境のみ追加定義（確定）
#define CS_FRAM_LOG  PD6  // FRAM 2号（ログ用・開発機専用・確定）
```
...
| # | 項目 | 状態 | 対応方針 |
|---|------|------|---------|
| 1 | FRAM 2号のCSピン | ✅確定 | PD6を試験環境専用として確定 |
| 2 | HIDレポートフォーマット | ✅確定 | 64B固定・cmd/seq/addr/length/data[56]の構造体で確定。USB Full Speedの物理制約（64B上限）による |
| 3 | ログのUSB転送方法 | ✅確定 | Phase 3 Task A（spi_flash_mgr HID通信実装）完了後にUSB HID経由でFRAM 2号のログをlog_capture.pyで吸い上げる。Task Aは実機試験より前に完了する |
| 4 | e-ink試験環境 | ⏳保留 | e-ink Display調達後に追記 |
| 5 | キースキャン試験 | ✅方針確定 | MCP23017ではなくseesawデバイス（ADA-5743・SKU9115・I2Cアドレス0x50）を使用。keyscan.cをseesaw対応に変更 |
...
| 2026-05 | 標準ブートローダと改変ブートローダのバイナリ解析完了（ジャンプ先変更は含まれないことを確認） |
