...
> - `CAR-01_emulator_dev_20260619.md`（エミュレータ仕様）
>
...
- WSL2（Windowsの場合）またはLinux環境
- ブラウザ（開発結果の確認に使用）

### 2.2 セットアップ手順

```bash
# 1. 依存パッケージのインストール（Ubuntu/WSL2の場合）
sudo apt update
...
ビルド済みのアプリストア版が公開された後は、アプリ内の開発者モードから直接ロードできるようになる予定です（現時点では上記の手動セットアップが必要です）。

### 2.3 動作確認画面

`serial_monitor.py` を起動した状態でブラウザから `panel.html` を開くと、TFT画面・OLED表示・デバッグログ・音声出力がまとめて確認できます。
...
```c
typedef enum {
    RENDER_BACKEND_WEBVIEW,  // デフォルト
    RENDER_BACKEND_NATIVE    // オプトイン（3.1節参照）
...
配布アプリ内では、ゲーム画面（TFT表示部分）の描画方式を2種類から選べます。OLED・メニュー・音声まわりはどちらを選んでも変わりません。

| バックエンド | 向いているジャンル | 開発者の負担 |
|---|---|---|
| `RENDER_BACKEND_WEBVIEW`（デフォルト） | ノベルゲーム・パズル・RPG・シミュレーション等、ほとんどのジャンル | 特になし。何も指定しなければこちらになります |
| `RENDER_BACKEND_NATIVE`（オプトイン） | 弾幕シューティングなど、1フレーム単位の反応速度が重要なジャンル | `Platform_Init()`で明示的に指定するだけ。描画パス自体はホストアプリ側が提供するため、ゲーム側で追加実装は不要です |

迷ったらデフォルト（WebView）のままで問題ありません。プレイ中に入力の遅れが気になる場合のみネイティブ版への切り替えを検討してください。
