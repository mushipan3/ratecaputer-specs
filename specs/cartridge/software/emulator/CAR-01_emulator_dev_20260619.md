...
SPI1バスにはTFT（CS=PC0）・SPI Flash（CS=PC4）・FRAM-1/2（CS=PD6/PD3）・YMF825（CS=PD2）が共有接続される。CSピンの状態に応じてルーティング先を切り替えるディスパッチャをプラグイン基盤として実装する（Phase 2 A-2-1）。
...
CAR-01完成を待たずにエンジン開発を先行するための抽象化レイヤー。Catacombsのオリジナルソースと同じ手法（`#ifdef _WIN32`）を採用。本改訂後の配布アーキテクチャ（16章、WebSocket+panel.html方式）とは独立した経路として併存させる。SDL2版はmushipan3のローカル開発・高速イテレーション用、WebSocket+panel.html版は配布・クロスプラットフォーム用という役割分担。
...
OLED・メニュー・システムUI・WebAudio音声は常にWebView経路で統一する（実装コスト・保守性を優先）。TFTゲーム画面の描画方式のみ、アプリ単位で以下の2方式から選択できるようにする。

| バックエンド | 方式 | 対象ジャンル | 工数 |
|---|---|---|---|
| `RENDER_BACKEND_WEBVIEW`（デフォルト） | canvas + 生ピクセル配列（putImageData） | ノベル・パズル・RPG等、大半のジャンル | 小（開発者は意識不要） |
| `RENDER_BACKEND_NATIVE`（オプトイン） | Android: SurfaceView直描画／iOS: Metal直描画 | 弾幕シューティング等、フレーム精度がシビアなジャンル | 中〜大（ホストアプリ側で1回実装すれば全アプリが利用可） |

ネイティブ描画パスはゲームごとに作るのではなく、「ネイティブ希望」と宣言したアプリが使える共通の高速パスとして、Android・iOSそれぞれに1実装ずつ用意する（Phase 6 / A-6-2・A-6-3で対応、17章参照）。アプリ側は`Platform_Init()`の引数でバックエンドを指定するだけでよい（13.2節）。

### 16.6 プラットフォーム別配布構成

```
開発時（mushipan3）：
  WSL2 Ubuntu 24.04でSpike+pluginを開発・検証

配布時（エンドユーザー）：
  Android: APK内にARM64ネイティブSpike同梱、WebView+panel.html
           （ネイティブ描画オプト時：SurfaceView直描画パスを追加同梱）
  iOS:     静的リンクしたSpike、WKWebView+panel.html、App Store配布
           （ネイティブ描画オプト時：Metal直描画パスを追加同梱）
  Windows: exe同梱、ブラウザまたは内蔵WebView
```
...
A-3-2  SPI1スタブへのYMF825組み込み（CS_YMF=PD2）
...
# 5. RV32EC動作確認用の最小テストバイナリ
cat > /tmp/test_rv32ec.S << 'EOF'
.section .text
.global _start
_start:
    li a0, 42
...
riscv64-unknown-elf-gcc \
  ...
  -nostdlib \
  -T /dev/stdin \
  -o /tmp/test_rv32ec.elf \
  /tmp/test_rv32ec.S << 'LDSCRIPT'
SECTIONS { . = 0x0; .text : { *(.text) } }
LDSCRIPT

# 6. Spikeで実行（exit code 42相当で終了すれば成功）
spike --isa=rv32ec \
  --mem=0x0:0x10000 \
  /tmp/test_rv32ec.elf
```

### 20.3 Phase 4・5（AVRエミュレータ）開発環境
...
Phase 6（配布向けネイティブアプリ化）が完了するまでの間、アプリ開発者はmushipan3と同じWSL2環境でビルド・実行する暫定運用とする。

```
1. 20.2節と同じ手順でWSL2+Spike環境を構築
2. car01_plugin（ビルド済みバイナリ配布予定）をロード
3. serial_monitor.py を起動
4. ブラウザでpanel.html を開く（Windows/WSL2いずれでも可）
...
| # | 項目 | 状態 |
|---|---|---|
| 1 | write_program.pyのHIDプロトコル詳細 | 🔒 実機待ち（UIAPduino） |
