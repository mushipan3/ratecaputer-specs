...
| mushipan3 | プラットフォーム本体・ファームウェアの開発 | 上記全て＋ATtiny404/AVR64DD28のCPUレベル完全エミュレーション、MCP73831充電ICの状態遷移シミュレーション、UPDIデバッグインターフェース |
...
```
          ...
          ┌──────────────┼──────────────────┐
          │              │                  │
    [A基板]ATtiny404   [D基板]AVR64DD28   CH1115 OLED
...
| D基板 | AVR64DD28 / CH1115 | 入力・日本語入力UI | I2Cスタブ（一般）／AVR完全エミュレーション（mushipan3） |
| A基板 | ATtiny404 / MCP73831 | 電源管理・バッテリー監視 | I2Cスタブ（一般）／AVR完全エミュレーション（mushipan3） |
...
| ATtiny404（A基板） | I2Cスタブ（一般）／AVRエミュレーション（mushipan3、詳細10章） | 高 | 要調査→解消済 |
| AVR64DD28（D基板） | I2Cスタブ（一般）／AVRエミュレーション（mushipan3、詳細10章） | 高 | 要調査→解消済 |
...
SPI FRAM（外部・スタブで模擬）：
  0x00000〜0x07FFF  SPI Flash管理処理（32KB・外部通信モード用・最も不変）
  0x08000〜0x09FFF  共通プログラムバックアップ（8KB・自己復元用）
  0x0A000〜0x0BFFF  管理領域（8KB・カタログキャッシュ・App Area配置状態表・IAP引数エリア等）
  0x0C000〜0x1BFFF  予備バッファ（64KB・旧恵梨沙フォント展開枠。Rev.0.6でSPI Flash直読み化し確保）
  0x1C000〜0x1FFFF  セーブデータ領域（16KB）
  0x20000〜        タスクスロット領域（メタ情報8KB＋CTXスタック16KB＋アプリ作業領域）
  ※恵梨沙フォントはFRAMに展開せずSPI Flash（0x008000）から直読み（Rev.0.6）
```
...
## 10. AVRエミュレーション（ATtiny404／AVR64DD28）

### 10.1 背景

D基板（ジョイスティック・日本語入力OLED）の音量調整コマンド処理、A基板（電源管理・バッテリー監視）の状態をエミュレータ上で再現する必要があるが、一般ユーザー・アプリ開発者にはCPUレベルの完全エミュレーションは不要で、I2C応答のスタブで十分という整理に至った。一方mushipan3にとってはATtiny404/1604ファームウェア自体の開発・検証が必須であり、CPUレベルのエミュレーションが必要。
...
★TODO(2026-08-09・要確認): 旧記述は ATtiny1604・ATtiny202（**ともに tinyAVR 0-series**）を前提としていた。**AVR64DD28 は AVR DD ファミリで別系統**のため、この分類はそのままでは成り立たない。simavr の対応可否を再確認すること。
ATtiny404 は tinyAVR 0-series（avrxmega3アーキテクチャ）に分類される。調査の結果、唯一実用的とされるsimavrがそもそもこの世代に対応していないことが判明し、**ライセンス問題以前にツールとして使えない**ことが分かった。これにより「自作するしかない」という結論が、ライセンス回避の都合ではなく技術的必然として確定した。

### 10.3 設計方針：二層構造

```
一般ユーザー／アプリ開発者向け（必須・今回実装）
  ATtiny404・AVR64DD28のCPUはエミュレート不要
  I2Cバスのふるまいのみスタブで再現すれば機能上十分

mushipan3向け（ファームウェア開発・検証用）
  ATtiny404・AVR64DD28のCPUを完全エミュレート
...
| デバイス | スタブ内容 |
|---|---|
| ATtiny404（バッテリー監視） | I2C Read要求に対し常に「満充電・USB未接続」相当の固定値を返す |
| AVR64DD28（ジョイスティック） | キーボード入力（WASD+Z+X+Enter）をJOYビットフィールドに変換してI2C経由で返答 |
| AVR64DD28→CH1115（OLED経由処理） | AVR64DD28 CPUをエミュレートせず、OLEDへの描画コマンドを直接panel.htmlの表示エリアにマッピング |
| 音量調整コマンド | AVR64DD28を経由せず、YMF825レジスタ0x19へ直接反映 |

### 10.5 mushipan3向け：自作MIT AVR8エミュレータ仕様

| 項目 | 内容 |
|---|---|
| 対象 | ATtiny404（4KB Flash/256B SRAM）、AVR64DD28（64KB Flash/8KB SRAM） |
| アーキテクチャ | tinyAVR 0-series（avrxmega3） |
| 命令セット | AVR8命令セット（Microchip公開データシートに基づくクリーンルーム実装） |
| 必須ペリフェラル（ATtiny404） | PA5 ADC（バッテリー電圧計算）、PA1 GPIO（MCP73831 STAT読み取り）、PA3 GPIO（MOSFET制御）、TCA0 PWM（BEEP）、TWI I2Cスレーブ |
| 必須ペリフェラル（AVR64DD28） | GPIO×5（JOY_UP/DOWN/LEFT/RIGHT/CENTER）、TWI I2Cスレーブ（対UIAPduino）、I2Cマスター（対CH1115） |
| ライセンス | MIT（自作のため制約なし） |
...
[A基板] ATtiny404（I2Cスレーブ）
    ↓ I2C Read（UIAPduinoがマスター）
[UIAPduino] CH32V003
    ↓ I2C Write（AVR64DD28スレーブへ）
[D基板] AVR64DD28
    ↓ I2C（CH1115スレーブへ）
...
| ATtiny404 I2C応答 | 常に100%固定返答 | ADCカーブ＋STAT込みの完全エミュレーション（10.5節のAVR8コア上で動作） |
| AVR64DD28→CH1115 | I2Cスタブ（直接描画） | AVR64DD28 CPUエミュレーション経由で描画 |
...
A-2-6  AVR64DD28 I2Cスタブ（ジョイスティック）
...
```
A-4-1  ATtiny404 I2Cスタブ（一般・開発者向け、満充電固定）
A-4-2  panel.html拡張④（バッテリーバー固定表示）

--- mushipan3向け追加実装 ---
A-4-3  MCP73831充電ICスタブ（3状態切り替え）
A-4-4  ATtiny404完全エミュレーション（自作MIT AVRエミュレータ）
A-4-5  電源シーケンス検証UI（panel.html拡張）
```

### Phase 5：AVRコアエミュレーション（mushipan3向け・AVR64DD28）
目標：AVR64DD28ファームウェアがCPUレベルで動く状態

```
A-5-1  自作MITミニAVRエミュレータ本体（Phase4実装を拡張）
A-5-2  AVR64DD28ペリフェラル追加（GPIO×5・TWI・I2Cマスター）
A-5-3  UPDIデバッガインターフェース
A-5-4  AVR64DD28ファームウェア動作確認
```
...
| I2C1 | ATtiny404/1604・CH1115向け | 高（本改訂で優先度引き上げ） |
...
| 自作MIT AVR8エミュレータ | MIT | ATtiny404/1604エミュレーション（mushipan3向け） | ソースに組み込み |
