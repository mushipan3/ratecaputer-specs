...
- CAR-01_emulator_dev_20260809.md §17 Phase 0〜2
...
| ジョイスティック（実機） | AVR64DD28 / I2C（boards_D §4.3） | keyscan_design §1 |
...
| 試験手順 | 1. keyscan_init()を呼び出す<br>2. 関数がハング・フォルトなく完了することを確認する<br>3. エミュレータ：AVR64DD28 I2Cスタブ(A-2-6)が応答すること |
| 試験データ | 入力なし |
...
| ★観測手段 | ~~**`tft_frame.bin` は使える**（…timeout 終了でも残る）~~ ★2026-08-14 是正: **この注記は誤り**。残るかどうかは timeout の問題ではなく、**その後の SWRESET が GRAM を消す**ため**ダンプのタイミング次第で内容が変わる**（本シナリオは最後に空カタログで赤画面→fail_stop→WDT リセットへ落ちる）。∴ `tft_frame.bin` を判定根拠にしてはならない（AI-0080）。UT-FBPS-02 は **FRAM トレース照合**へ移した。。`fram*.bin` と扱いが違う点に注意 |
...
|---|---|
| 試験ID | IT-RSRC-03 |
| 試験名 | 512エントリ超のアプリでディレクトリ拡張ブロック(res_type=0xF)から検索できることの確認 |
| 対応仕様 | cartridge_master §1.8（ディレクトリ拡張）★**2026-08-14 にディレクトリ拡張ブロックの種別を 0xE → 0xF へ移した**（AI-0064・パッチテーブルが 0x6→0xE へ移動したため） |
| 前提条件 | IT-RSRC-01 PASS。SPI FlashにブロックN+1以降のリソースが存在するダミーアプリが格納されていること。 |
| 試験手順 | 1. res_type_id=0x1_200（ID=0x200=512番目の画像）でload_resource()を呼び出す<br>2. 拡張ブロック(res_type=0xF)を辿って正しいリソースに到達することを確認する<br>3. 転送先FRAMに正しいデータが書き込まれることを確認する |
...
**前提:** CAR-01_emulator_dev_20260809.md Phase 0〜2 完了済み（A-0-1〜A-2-9）
...
□ キー入力デバイス（AVR64DD28 / I2C、boards_D §4.3）
